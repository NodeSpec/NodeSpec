...
**[nodespec.io](https://nodespec.io)** · Run it locally in minutes: see [Install](#install).

---

## The problem it solves

AI coding assistants are fast and the frontier models have become extremely powerful with reasoning ability. All advancements considered, the tendency for sprawl
...
- **Test Driven Methodology**: Just enough test driven development (TDD) practices to ensure acceptance criteria only flip to met when your model or separate test bench proves them with reported results.
...
| Edition | For |
|---|---|
| **Community** | Free open source download (this repo, unlimited projects) or the hosted free tier at nodespec.io (2 projects). Every feature, no credit card. Your AI does the building over MCP. |
| **Indie** | Unlimited hosted projects, Repo Import for faster brownfield development |
...
service — not a fork. It runs the same schema, the same MCP server, and the
same evidence loop. See [nodespec.io/pricing](https://nodespec.io/pricing) for
current details.
...
Everything runs with Docker alone — the same two commands on Windows, macOS,
and Linux. No WSL, no shell scripts, no extra CLIs.
...
cd NodeSpec/deploy/community
cp .env.example .env
```

In `.env`, set the one required value: `ENCRYPTION_SECRET`, any long random
string — for example the output of `openssl rand -base64 32`, or on Windows
PowerShell:
...
The first run downloads several gigabytes of container images and builds the
frontend — allow a few minutes. `docker compose ps` shows progress; everything
except `db-init` (a one-shot that runs once and exits) should reach "running".
Then open http://localhost and create your account — accounts confirm
instantly on a local install, no e-mail round-trip.

[`deploy/community/README.md`](deploy/community/README.md) covers everyday
commands, promoting your first admin, updating, and the checklist for
exposing a deployment beyond your own machine (regenerate the demo secrets,
disable local trust, put TLS in front).
...
- Create your NodeSpec account **before** connecting your AI — with
  `MCP_LOCAL_TRUST=true` (the default), a single-user install approves the
  connection against your account with no browser sign-in. This stands down
  automatically once a second account exists or the account enrolls
  two-factor auth; set it to `false` before exposing the deployment beyond
  your machine.
- Some clients refuse plain-HTTP server URLs (Claude Desktop's connector
  settings among them). For those, the in-app guide shows a small bridge
  configuration (`mcp-remote`) that works without TLS. Note that Claude
  Desktop only loads MCP servers at launch: after any config change, fully
  quit it (system tray → Quit) and reopen.
- The very first AI call warms a dependency cache and can take up to 30
  seconds; everything after is fast.
...
The database volume survives updates; the init step never touches an
initialized database. Back up before major updates — the deploy guide shows
how.
